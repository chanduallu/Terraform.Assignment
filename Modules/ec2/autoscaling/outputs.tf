output "this_launch_configuration_id" {
  ...
  value       = var.launch_configuration == "" && var.create_lc ? element(concat(aws_launch_configuration.this.*.id, [""]), 0) : var.launch_configuration
}

output "this_launch_configuration_name" {
  description = "The name of the launch configuration"
  value       = var.launch_configuration == "" && var.create_lc ? element(concat(aws_launch_configuration.this.*.name, [""]), 0) : ""
}
...
output "this_autoscaling_group_desired_capacity" {
  ...
  value = element(
    concat(aws_autoscaling_group.this.*.desired_capacity, [""]),
    0,
  )
}
...
output "this_autoscaling_group_default_cooldown" {
  ...
  value = element(
    concat(aws_autoscaling_group.this.*.default_cooldown, [""]),
    0,
  )
}
...
output "this_autoscaling_group_health_check_grace_period" {
  ...
  value = element(
    concat(aws_autoscaling_group.this.*.health_check_grace_period, [""]),
    0,
  )
}
...
output "this_autoscaling_group_health_check_type" {
  ...
  value = element(
    concat(aws_autoscaling_group.this.*.health_check_type, [""]),
    0,
  )
}
